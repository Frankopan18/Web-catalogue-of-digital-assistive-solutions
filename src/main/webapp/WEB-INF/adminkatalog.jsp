	...
	<div align="center">
		<form action="" method="put" style="width: 700px; margin: 0 auto;">
			<input type="text" name="searchbox" id="myInput"
				onkeyup="myFunction()"
				Placeholder="Unesi digitalno asistivno rješenje(Ime).."
		...
		<br /> <br />
		<form action="/onlinecatalogue/dodajTech" method="get">
			<button type="Submit">Dodaj rješenje</button>
		</form>
		<form action="/onlinecatalogue/urediKorisnik.jsp" method="get">
			<button type="Submit">Uredi korisnike</button>
		</form>
		<form action="/onlinecatalogue/urediAdministrator.jsp" method="get">
			<button type="Submit">Uredi administratore</button>
		</form>
		<br />
		<table cellpadding="5" id="myTable" class="table table-bordered">
			<th>Naziv</th>
			<th>Proizvođač</th>
			<th>Mjesto primjene</th>
			<th>Vrsta teškoće</th>
			<th>Link</th>
			<th>Jezici</th>
			<th>Uređivanje</th>
			...
			<c:forEach var="itemCatalog" items="${listCatalogs.rows}">
				...
				<tr>
					<td>${itemCatalog.Naziv}</td>
					<td>${itemCatalog.Proizvodjac}</td>
					<td>${itemCatalog.mjestoPrimjene}</td>
					<c:set var="string1" value="${itemCatalog.vrstaPoteskoce}" />
					<c:set var="string2"
						value="${fn:replace(string1, 'Poteškoće', 'Teškoće')}" />
					<c:set var="string3"
						value="${fn:replace(string2, 'poteškoće', 'teškoće')}" />
					<td>${string3}</td>
					<td><a href="${itemCatalog.Link}" target="_blank">${itemCatalog.Link}</a></td>
					<td>${itemCatalog.Jezici}</td>
					<td>
						<form
							action="/onlinecatalogue/urediTehnologiju?id=${itemCatalog.ID}"
							method="post">
							<button type="Submit"
								style="background-color: transparent; border-color: transparent; float: left; left: 0px; width: 10px;">
								<font color="white">Uredi</font>
							</button>
						</form>
					</td>
					<td>
						<form action="/onlinecatalogue/izbrisi?id=${itemCatalog.ID}"
							method="post">
							<button type="Submit"
								style="background-color: transparent; border-color: transparent; float: left; left: 0px; width: 10px;">
								<font color="white">Obriši</font>
							</button>
						</form>
					</td>
				</tr>
